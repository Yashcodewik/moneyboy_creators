  ...
  return (
    <div className="modal show" role="dialog" aria-modal="true" aria-labelledby="age-modal-title">
      <form className="modal-wrap report-modal" onSubmit={formik.handleSubmit}>
        <button type="button" className="close-btn" onClick={() => onClose(false)}><CgClose size={22} /></button>
        ...
        <div>
          ...
          <CustomSelect searchable={false} label="Select Report Option" value={formik.values.title} onChange={(value: string | string[]) => {const val = Array.isArray(value) ? value[0] : value; formik.setFieldValue("title", val); formik.setFieldTouched("title", true);}}
            options={[
        ...
        <div className="input-wrap">
          ...
          <textarea rows={3} placeholder="Tell Us Why You Report?" value={formik.values.description} onChange={formik.handleChange} name="description" />
