...
interface WishlistMediaCardProps {
  id: number; image: string; mediaType?: "image" | "video"; description: string; price?: string; likes: number; views: number; isSubscriberOnly?: boolean; isLiked?: boolean; onLike?: (id: number) => void; isSaved?: boolean; onToggleSave?: (id: number) => void; onUnlock?: () => void; onSubscribe?: () => void;
}
const WishlistMediaCard = ({ id, image, description, price = "$10.00", mediaType, likes, views, isSubscriberOnly = false, isLiked = false, isSaved = false, onLike, onToggleSave, onUnlock, onSubscribe }: WishlistMediaCardProps) => {
          ...
          <div className="creator-content-card__media">
            ...
            <div className="creator-media-card__overlay">
              <div className="creator-media-card__stats">
                <Link href="#" className={`creator-media-card__stats-btn wishlist-icon ${isSaved ? "active" : ""}`} onClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleSave?.(id); }}>
