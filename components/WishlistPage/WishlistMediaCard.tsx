...
interface WishlistMediaCardProps {id: number; image: string; description: string; price?: string; likes: number; views: number; isSubscriberOnly?: boolean; isLiked?: boolean; onLike?: (id: number) => void;}
const WishlistMediaCard = ({id, image, description, price = "$10.00", likes, views, isSubscriberOnly = false, isLiked = false, onLike,}: WishlistMediaCardProps) => {
  const [liked, setLiked] = useState(isLiked);
  const handleLike = () => {setLiked((prev) => !prev); onLike?.(id);};
  return (
    <div className="creator-content-card-container profile_card wishlist_card">
      <div className="creator-content-card">
        <div className="creator-content-card__media">
          <div className="creator-card__img"><img src={image} alt="Creator Content" /></div>
          <div className="creator-media-card__overlay">
            <div className="creator-media-card__stats">
              <Link href="#" className="creator-media-card__stats-btn wishlist-icon active">
        ...
        <div className="desc-btns">
          <div className="creator-content-card__description"><p>{description}</p></div>
