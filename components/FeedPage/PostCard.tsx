      ...
      <div className="moneyboy-post__container card">
        ...
        <div className="moneyboy-post__media">
          ...
          <div className="moneyboy-post__actions tooltip_wrapper">
            <ul>
              {/* Send Tip */}
              <li>
                <Link href="#" data-tooltip="Send Tip" onClick={(e) => { e.preventDefault(); if (isOwnPost) return; if (!session?.user?.id) { router.push("/login"); return; } session?.user?.id !== post.userId && setShowTipModal(true); }}>
              ...
              {/* Like */}
              <li>
                <Link data-tooltip="Like" href="#" className={`post-like-btn stable-like-btn ${post.isLiked ? "active" : ""}`} onClick={async (e) => { e.preventDefault(); await onLike(post._id); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              {/* Comment */}
              <li>
                <Link data-tooltip="Comment" href="#" onClick={(e) => { e.preventDefault(); if (!session?.user?.id) { router.push("/login"); return; } setShowComment((prev) => !prev); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <ul>
              {/* Share */}
              <li>
                <Link data-tooltip="Report" href="#" className={isReported ? "active" : ""} onClick={(e) => { e.preventDefault(); if (isOwnPost) return; if (!session?.user?.id) { router.push("/login"); return; } if (isReported) return; session?.user?.id !== post.userId && setShowReportModal(true); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              {/* Save */}
              <li>
                <Link data-tooltip="Wishlist" href="#" className={`post-save-btn ${post.isSaved ? "active" : ""}`} onClick={(e) => { e.preventDefault(); if (isOwnPost) return; session?.user?.id !== post.userId && onSave(post); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        ...
        {showComment && (
          <div className="flex flex-column gap-20">
            ...
            {topComment && (
              <div className="fdpost_comment gap-15">
                <div className="moneyboy-post__header">
