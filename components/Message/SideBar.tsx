...
const MuteIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
    ...
    <path d="M21.1481 7.79999C22.6181 11.29 22.1781 15.37 19.8281 18.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M22 2L2 22" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);

interface ChatItemProps { chat: any; isActive?: boolean; onClick: () => void; gradientId: string; showDisplayName?: boolean; }
const ChatItem = ({ chat, isActive, onClick, gradientId, showDisplayName = false, }: ChatItemProps) => (
  <div className="msg-contact-box" msg-chat-contact="" data-active={isActive ? "true" : undefined} onClick={onClick}>
    <div className="contact-item">
      <div className="contact-avatar-wrapper">
        {chat?.user?.image ? (
          <img className="contact-avatar" src={chat.user.image} alt="" />
        ) : (
          <div className="noprofile"><LogoSVG id={gradientId} /></div>
        )}
        <div className={`contact-status-indicator ${chat?.user?.isOnline ? "online" : "offline"}`} />
      </div>
      <div className="contact-content">
        <div className="contact-header">
          <h4 className="contact-name">{showDisplayName ? chat?.user?.displayName || chat?.user?.username || "Unknown User" : chat?.user?.username || "Unknown User"}</h4>
          <span className="contact-time">{chat?.updatedAt ? new Date(chat.updatedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", }) : ""}</span>
        </div>
        <div className="contact-preview">
          <div className="contact-message">
            <p>{chat?.lastMessage?.message || chat?.lastMessage || ""}</p>
          </div>
          {chat?.isMuted && (<div className="icon mute-icon"><MuteIcon /></div>)}
          {chat?.unreadCount > 0 && (<div className="contact-unread-count"><span>{chat.unreadCount}</span></div>)}
        </div>
      </div>
    </div>
  </div>
...
const SideBar = ({ onSelectChat, activeThreadId }: any) => {
  ...
  const fetchUsers = async (page = 1, append = false) => {
    ...
    try {
      ...
      const newUsers: any[] = res?.data || [];
      setUserResults((prev) => (append ? [...prev, ...newUsers] : newUsers));
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (isNearBottom && hasMoreUsers && !loadingUsers) {
  ...
  const filterChats = (list: any[]) => {
    ...
    return list.filter((chat) => {
      ...
      const lastMessage = (chat?.lastMessage?.message || chat?.lastMessage || "").toLowerCase(); return username.includes(lower) || lastMessage.includes(lower);
    });
  };
  ...
  const filteredHiddenChatList = useMemo(
    ...
    [hiddenChatList, searchText]
  );

  useEffect(() => {
    const handleOnline = ({ userId }: any) => dispatch(updateUserOnlineStatus({ userId, isOnline: true }));
    const handleOffline = ({ userId }: any) => dispatch(updateUserOnlineStatus({ userId, isOnline: false }));
  ...
  setHasMoreUsers(true);
    ...
    try {
      const res = await apiPost({
        url: "messages/thread",
        values: { receiverId: user._id || user.id },
      });
      if (res?.threadId) {
    ...
    <div className="msg-profiles-layout">
      <div className="msg-profiles-header">
        <div className="msg-profiles-header-container">
          <div className="msg-layout-title"><h2>Messages</h2></div>
          <div className="msg-profiles-search-input searchsuggest_wraping">
            ...
            {userSearchText && (userResults.length > 0 || loadingUsers) && (
              <div className="search-users scrolling" onScroll={handleScroll}>
                ...
                {userResults.map((user) => (
                  <div key={user.publicId} className="user_wrap" onClick={() => handleUserClick(user)}>
                    <div className="user_avatar">
                      {user?.profile ? (
                        <img className="userimg" src={user.profile} alt="user" />
                      ) : (
                        <div className="noprofile"><LogoSVG id={`search-grad-${user.publicId}`} /></div>
                      )}
                    </div>
                ...
                {loadingUsers && userResults.length > 0 && (
                  <div className="loadingtext loadingtext--bottom">
      ...
      <div className="msg-profiles-wrapper">
        <div className="msg-profiles-container" msg-chat-contacts-wrapper="">
          {filteredHiddenChatList.length > 0 && (
