  ...
  return (
    <Modal show={true} onClose={onClose} size="md" title="Edit Image" className="imgcrop_wrap">
      <form className="modal_containt imgcrop-modal" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="actions mt-3"><button className="premium-btn active-down-effect" type="button" onClick={handleSave} disabled={loading}><span>{loading ? "Processing..." : "Save"}</span></button></div>
      </form>
    </Modal>
  );
}
