...
const PPVRequestModal = ({ show, onClose, creator, post, onSuccess, }: PPVRequestModalProps) => {
  ...
  return (
    <>
      <Modal show={show} onClose={onClose} title="PPV - Request Custom Content" size="lg" className="request_wrap">
        <div className="modal_containt request-modal">
          ...
          <div>
            ...
            <CustomSelect searchable={false} label="Request type" value={formik.values.requestType} onChange={(value) => formik.setFieldValue("requestType", value)} options={[{ label: "Video", value: "VIDEO" }, { label: "Photo", value: "PHOTO" },]} />
            {formik.touched.requestType && formik.errors.requestType && (<span className="error-message">{formik.errors.requestType}</span>)}
          </div>
          <div>
            <label>Description</label>
            <textarea rows={3} placeholder="Describe what you’d like (tone, outfit, duration, etc.)" value={formik.values.description} onChange={formik.handleChange} onBlur={formik.handleBlur} name="description" />
          </div>
          ...
          <div>
            ...
            <div className="upload-wrapper">
              ...
              {!mediaPreview && (
                <div className="img_wrap">
                  <svg className="icons idshape size-45" onClick={() => document.getElementById("ppv-file-input")?.click()} />
                </div>
              )}
              <input id="ppv-file-input" type="file" accept="image/*,video/*" style={{ display: "none" }}
                onChange={(e) => {
                  const selectedFile = e.target.files?.[0];
                  if (!selectedFile) return;
                  const isImage = selectedFile.type.startsWith("image/");
                  const isVideo = selectedFile.type.startsWith("video/");
                  if (formik.values.requestType === "PHOTO" && !isImage) { showError("You selected Request type Photo. Please upload an image for Photo request"); return; }
                  if (formik.values.requestType === "VIDEO" && !isVideo) { showError("You selected Request type Video. Please upload a video for Video request"); return; }
          ...
          <div>
            ...
            <div className="select_wrap w-full">
          ...
          {paymentMethod === "card" &&
            <div>
              ...
              <CustomSelect label="Select a Payment Card" placeholder="Select a Payment Card" value={selectedCard} options={cards.map((card) => ({ label: `${card.cardholderName} - **** ${card.cardNumber}`, value: card._id, }))} onChange={(val: any) => { console.log("SELECTED VALUE:", val); setSelectedCard(val); }} />
            </div>
          }
          <div className="actions">
            <button className="premium-btn active-down-effect" onClick={() => formik.handleSubmit()} disabled={loading}><span>{loading ? "Sending..." : "Send Request"}</span></button>
