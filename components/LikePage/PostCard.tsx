...
export interface ApiCreatorInfo {
  userName: string;
  displayName: string;
  profile: string;
  publicId: string;
    ...
    <>
      <div className="moneyboy-post__container card">
        <div className="moneyboy-post__header">
          <div className="profile-card" onClick={() => handleProfileClick(post.creatorInfo?.userName)}>
            <div className="profile-card__main">
              <div className="profile-card__avatar-settings">
                <div className="profile-card__avatar">
                  {post?.creatorInfo?.profile?.trim() ? (
                    <img src={post.creatorInfo.profile} alt={post.creatorInfo?.displayName || "User Avatar"} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                  ) : (
                    ...
                    <button type="button" ref={tagButtonRef} className="active-down-effect" onClick={() => setShowTaggedUsers((prev) => !prev)}>
                      <ul className="taglist">
                        {post.taggedCreators.slice(0, 3).map((user: any) => (
                          <li key={user._id}>
                            <img src={user.profile} className="user_icons" alt={user.userName} />
                          </li>
                        ))}
                        {post.taggedCreators.length > 3 && (
                          <li className="more-count">+{post.taggedCreators.length - 3}</li>
                        )}
                      </ul>
                    </button>
                  )}
          ...
          <div className="moneyboy-post__upload-more-info">
            <div className="moneyboy-post__upload-time">{formatRelativeTime(post.createdAt)}</div>
        ...
        <div className="moneyboy-post__desc pointer" onClick={handlePostRedirect}>
        ...
        <div className="moneyboy-post__media">
          ...
          <div className="moneyboy-post__actions tooltip_wrapper">
            <ul>
              <li>
                <Link href="#" data-tooltip="Send Tip" onClick={(e) => { e.preventDefault(); setShowTipModal(true); }}>
              ...
              <li>
                <Link href="#" data-tooltip="Like" className={`post-like-btn ${post.isLiked ? "active" : ""}`} onClick={(e) => { e.preventDefault(); onLike(post._id); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <ul>
              <li>
                <Link href="#" data-tooltip="Report" className={isReported ? "active" : ""} onClick={(e) => { e.preventDefault(); if (post.isReported) return; setShowReportModal(true); }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              <li>
                <Link href="#" data-tooltip="Wishlist" className={`post-save-btn ${saved ? "active" : ""}`} onClick={async (e) => { e.preventDefault(); const newValue = !saved; setSaved(newValue); try { await onSave(post._id, saved); } catch { setSaved(!newValue); } }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
