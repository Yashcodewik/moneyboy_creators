...
export default function FeaturedContentSlider({
  ...
  return (
    <Swiper modules={[Navigation]} spaceBetween={16} slidesPerView={2} breakpoints={{0: {slidesPerView: 1,}, 768: {slidesPerView: 2,},}} navigation={{prevEl: ".prev-btn", nextEl: ".next-btn",}}>
      {featuredPosts?.map((post: any) => {
        const image = post.media?.[0]?.mediaFiles?.[0] || "/images/profile-avatars/profile-avatar-21.jpg";
              ...
              <div className="featured-content-premium-card-container">
                <div className="featured-content-bg-img">
                  ...
                  !mediaErrors[post._id] ? (
                    <video src={post.media[0].mediaFiles[0]} muted playsInline preload="metadata" onError={() => setMediaErrors((prev) => ({...prev, [post._id]: true,}))}/>
                  ) : post.media?.[0]?.mediaFiles?.[0] &&
                    !mediaErrors[post._id] ? (
                    <img src={post.media[0].mediaFiles[0]} alt="Featured Content" onError={() => setMediaErrors((prev) => ({...prev, [post._id]: true,}))}/>
                  ) : (
                ...
                <div className="featured-premium-card-content-container">
                  <div className="featured-premium-card-content">
                    ...
                    <p>{post.text}</p>
                    {post.isUnlocked && (
                      <button className="btn-txt-gradient btn-outline grey-variant" onClick={() => handlePostRedirect(post)}><span>Purchased</span></button>
                    )}
                    {/* SUBSCRIBED */}
                    {!post.isUnlocked &&
                      post.isSubscribed &&
                      post.accessType === "subscriber" && (
                        <button className="btn-txt-gradient btn-outline grey-variant" onClick={() => handlePostRedirect(post)}><span>Subscribed</span></button>
                      )}

                    {/* PAY PER VIEW */}

                    {!post.isUnlocked && post.accessType === "pay_per_view" && (
                      <button className="btn-txt-gradient btn-outline" onClick={() => onUnlock(post)} disabled={isOwnPost}>
                    ...
                    {!post.isUnlocked && !post.isSubscribed && post.accessType === "subscriber" && (
                        <button className="btn-txt-gradient btn-outline grey-variant" onClick={onSubscribe} disabled={isOwnPost}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                            ...
                            <defs>
                              <linearGradient id="paint0_linear_745_209" x1="9.9704" y1="1.65417" x2="9.9704" y2="15.8167" gradientUnits="userSpaceOnUse"> <stop stop-color="#FFCD84"></stop>{" "} <stop offset="1" stop-color="#FEA10A"></stop>
                              </linearGradient>
                              <linearGradient id="paint1_linear_745_209" x1="9.99935" y1="18.3333" x2="9.99935" y2="19.3333" gradientUnits="userSpaceOnUse"> <stop stop-color="#FFCD84"></stop>{" "} <stop offset="1" stop-color="#FEA10A"></stop>
                              </linearGradient>
