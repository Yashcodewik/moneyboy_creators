...
const StorePage = () => {
  ...
  useEffect(() => {
    ...
    dispatch(
      fetchMyPaidPosts({ page: 1, limit: 8, search, time, publicId: activeStoreOwner.publicId, type: subActiveTab === "videos" ? "video" : "photo", }),
    );
    ...
    <>
                  ...
                  <div className="st_head">
                    <div className="head_text">
                      ...
                      <div className="btn-controls">
                        <button className="moneyboy-swiper-control-btn next-btn" onClick={handlePrev} disabled={creatorsPagination.page === 1 || loadingCreators}><ChevronLeft size={18} /></button>
                        <button className="moneyboy-swiper-control-btn next-btn" onClick={handleNext} disabled={!creatorsPagination.hasNextPage || loadingCreators}><ChevronRight size={18} /></button>
                      </div>
                    </div>
                    <ul>
                                ...
                                {activeStoreOwner.profile ? (
                                  ...
                                  <div className="noprofile">
                                    <svg width="40" height="40" viewBox="0 0 66 54" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      ...
                                      <defs>
                                        <linearGradient id="paint0_linear_4470_53804" x1="0"
                                          y1="27"
                                          x2="66"
                                          y2="27"
                                          gradientUnits="userSpaceOnUse"
                  ...
                  <div className="hero-type-card-container">
                    ...
                    <div className="hero-type-card--content-container">
                      ...
                      <div className="hero-type-card--cta-box">
                        ...
                        <div>
                          <button disabled={isOwnStore} className="btn-txt-gradient shimmer btn-outline p-sm" onClick={() => setShowPPVModal(true)}><span>Request PPV</span></button>
                        </div>
                      </div>
                      <div className="hero-type-card--desc">
                      ...
                      <div className="creator-content-filter-grid-container" data-multiple-tabs-section>
                        ...
                        <div className="creator-content-cards-wrapper multi-dem-cards-wrapper-layout store_card" data-layout-toggle-rows={layout === "list" ? true : undefined}>
                          {subActiveTab === "videos" && (
                            <div className="creator-content-type-container-wrapper" data-multi-tabs-content-tabdata__active>
                              ...
                              <div className="col-4-cards-layout">
                                {!loadingPaidPosts && paidPosts.filter((post) => getPostMedia(post).type === "video",).map((post) => {
                                  const media = getPostMedia(post); const isOwnPost = isCreator && post.userId === loggedInUserId; const isSaved = post.isSaved;
                                  return (
                                    <div className="creator-media-card card" key={post._id}>
                                            ...
                                            {!isOwnPost && !post.isUnlocked && !post.isSubscribed && (
                                              <div className={`creator-media-card__stats-btn wishlist-icon ${isSaved ? "active" : ""}`} onClick={(e) => handleSaveToggle(e, post,)}>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                  ...
                                                  <path d="M16.8199 2H7.17995C5.04995 2 3.31995 3.74 3.31995 5.86V19.95C3.31995 21.75 4.60995 22.51 6.18995 21.64L11.0699 18.93C11.5899 18.64 12.4299 18.64 12.9399 18.93L17.8199 21.64C19.3999 22.52 20.6899 21.76 20.6899 19.95V5.86C20.6799 3.74 18.9499 2 16.8199 2Z" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                                  <path d="M9.25 9.04999C11.03 9.69999 12.97 9.69999 14.75 9.04999" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                                </svg>
                                              </div>
                                            )}
                                          </div>
                                      ...
                                      <div className="creator-media-card__desc"><p>{post.text} 123</p></div>
                                      <div className="creator-media-card__btn">
                                        <>
                                          {post.isUnlocked && (<a className="btn-txt-gradient shimmer btn-outline grey-variant" onClick={() => handlePostRedirect(post, isOwnPost,)}><span>Purchased</span></a>)}
                                          {/* SUBSCRIBED */}
                                          {!post.isUnlocked && post.isSubscribed && post.accessType === "subscriber" && (
                                            <Link href="#" className="btn-txt-gradient shimmer btn-outline grey-variant" onClick={() => handlePostRedirect(post, isOwnPost,)}><span>Subscribed</span></Link>
                                          )}
                                          {/* PAY PER VIEW */}
                                          {!post.isUnlocked &&
                                            post.accessType ===
                                            "pay_per_view" && (
                                              <Link href="#" className="btn-txt-gradient shimmer btn-outline" onClick={(e) => { if (isOwnPost) { e.preventDefault(); return; } setUnlockModalPost(post); }}>
                                          ...
                                          {!post.isUnlocked && !post.isSubscribed && post.accessType === "subscriber" && (
                                            <Link href="#" className="btn-txt-gradient shimmer btn-outline grey-variant" onClick={(e) => { if (isOwnPost) { e.preventDefault(); return; } setSubscriptionPlan("MONTHLY",); setShowSubscriptionModal(true,); }}>
                                              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                                <path d="M13.9173 15.8167H6.08399C5.73399 15.8167 5.34232 15.5417 5.22565 15.2083L1.77565 5.55834C1.28399 4.17501 1.85899 3.75001 3.04232 4.60001L6.29232 6.92501C6.83399 7.30001 7.45065 7.10834 7.68399 6.50001L9.15065 2.59167C9.61732 1.34167 10.3923 1.34167 10.859 2.59167L12.3257 6.50001C12.559 7.10834 13.1757 7.30001 13.709 6.92501L16.759 4.75001C18.059 3.81667 18.684 4.29168 18.1507 5.80001L14.784 15.225C14.659 15.5417 14.2673 15.8167 13.9173 15.8167Z" stroke="url(#paint0_linear_745_209)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                                <path d="M5.41602 18.3333H14.5827" stroke="url(#paint1_linear_745_209)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                                <path d="M7.91602 11.6667H12.0827" stroke="url(#paint2_linear_745_209)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                                                <defs>
                          ...
                          <div className="creator-content-type-container-wrapper" data-multi-tabs-content-tab>
                            {subActiveTab === "photos" && (
                              <div className="col-4-cards-layout">
                                {paidPosts.filter((post) => getPostMedia(post).type === "photo",)
                                  .map((post) => {
                                    const media = getPostMedia(post); const isOwnPost = isCreator && post.userId === loggedInUserId; const isSaved = post.isSaved;
                                    return (
                                      <div className="creator-media-card card" key={post._id}>
                                        <div className="creator-media-card__media-wrapper">
                                          <div className="creator-media-card__media"><img src={media.url} alt="Post Image" /></div>
                                          <div className="creator-media-card__overlay">
                                            <div className="creator-media-card__stats">
                                              {!isOwnPost && !post.isUnlocked && !post.isSubscribed && (
                                                <div className={`creator-media-card__stats-btn wishlist-icon ${isSaved ? "active" : ""}`} onClick={(e) => handleSaveToggle(e, post)}>
                                        ...
                                        <div className="creator-media-card__btn mt-auto">
                                          <>
                                            {post.isUnlocked && (<a className="btn-txt-gradient shimmer btn-outline grey-variant" onClick={() => handlePostRedirect(post, isOwnPost,)}><span>Purchased</span></a>)}
                                            {!post.isUnlocked && post.isSubscribed && post.accessType === "subscriber" && (<a className="btn-txt-gradient shimmer btn-outline grey-variant" onClick={() => handlePostRedirect(post, isOwnPost,)}><span>Subscribed</span></a>)}
      ...
      <div className={`modal ${open ? "show" : ""}`} role="dialog" aria-modal="true">
        <div className="modal-wrap video_modal">
          ...
          <Plyr options={{ controls: ["play-large", "play", "progress", "current-time", "mute", "fullscreen",], }} source={{ type: "video", sources: [{ src: "https://res.cloudinary.com/drhj03nvv/video/upload/v1770026049/posts/69807440e60b526caa6da50c/1770026048286-screen-capture.webm.mkv", type: "video/webm", },], }} />
        </div>
      </div>
