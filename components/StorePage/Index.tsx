...
const StorePage = () => {
  ...
  const { loadingCreators } = useSelector((state: RootState) => state.creators);
  const isAppendingCreators = useRef(false);
        ...
        <div className="moneyboy-2x-1x-a-layout">
          <div className="moneyboy-feed-page-container moneyboy-diff-content-wrappers">
            ...
            <BtnGroupTabs activeTab={activeMainTab} onChange={(value) => {
              setActiveMainTab(value as MainTab); setSelectedCreator(null); setCreatorPage(1); const params = new URLSearchParams(searchParams.toString()); params.set("tab", value); router.push(`/store?${params.toString()}`);
            }}
                  ...
                  <div className="st_head">
                    <div className="head_text">
                      <h5 className="flex items-end justify-center">
                        <img src="/images/logo/profile-badge.png" alt="M Icons" className="max-w-22" />
                        {" "}Creators
                      </h5>
...
onInit={(swiper) => {
 ...
 onSlideChange={(swiper) => { setIsBeginning(swiper.isBeginning); setIsEnd(swiper.isEnd); }} onReachEnd={() => { if (creatorPage < (creatorsPagination?.totalPages ?? 1)) { setCreatorPage((prev) => prev + 1); } }} onReachBeginning={() => setIsBeginning(true)} slidesPerView="auto" spaceBetween={0} freeMode={{ enabled: true, momentum: true, momentumRatio: 0.5 }} watchOverflow={false} observer observeParents simulateTouch grabCursor modules={[FreeMode]} className="creators-swiper">
                      {loadingCreators && !isAppendingCreators.current ? Array.from({ length: 14 }).map((_, i) => (
                        <SwiperSlide key={`initial-skeleton-${i}`}>
                          <div className="creator-item loading-slide">
                            <div className="icons_wrap"><img className="noprofile skeleton-pulse" /></div>
                            <span className="skeleton-pulse" style={{ width: "48px", height: "10px", borderRadius: "4px", }} />
                          </div>
                        </SwiperSlide>
                      ))
                      ...
                      {loadingCreators && isAppendingCreators.current && Array.from({ length: 3 }).map((_, i) => (
                        <SwiperSlide key={`append-skeleton-${i}`}>
                          <div className="creator-item loading-slide">
                            <div className="icons_wrap"><div className="noprofile skeleton-pulse" /></div>
                            <span className="skeleton-pulse" style={{ width: "48px", height: "10px", borderRadius: "4px", }} />
                          </div>
                        </SwiperSlide>
                      ))}
                ...
                <div className="moneyboy-special-content-banner-wrapper card">
                  <div className="moneyboy-special-content-banner-container">
                    <div className="moneyboy-special-content-banner--content">
                      ...
                      <a href="#" className="btn-txt-gradient shimmer btn-outline" onClick={(e) => { e.preventDefault(); if (isOwnStore || isSubscribed) return; handleSubscribeClick(); }}>
                        <LockIcon />
                    ...
                    <div className="moneyboy-special-content-banner--img-wrapper">
                      ...
                      {(storeImages.profileImage || activeStoreOwner?.profile) && !specialBannerError ? (
                        <img src={storeImages.profileImage || activeStoreOwner.profile} alt={`${activeStoreOwner.displayName || "User"} Profile`} onError={() => setSpecialBannerError(true)} />
                      ) : (
                        <NoProfilePlaceholder />
                      )}
                              ...
                              <div className="multi-tabs-action-buttons">
                                {(Object.keys(tabConfig) as SubTab[]).map((tab) => {
                                  const config = tabConfig[tab]; return (
                                    <button key={tab} className={`multi-tab-switch-btn ${tab}-btn ${subActiveTab === tab ? "active" : ""}`} onClick={() => setSubActiveTab(tab)}> {config.icon} <span>{config.label}</span> </button>
                                  );
                                })}
                              </div>
                            </div>
      ...
      {unlockModalPost && (
        <UnlockContentModal onClose={() => setUnlockModalPost(null)} creator={{ displayName: unlockModalPost.creatorInfo?.displayName || unlockModalPost.user?.displayName, userName: unlockModalPost.creatorInfo?.userName || unlockModalPost.user?.userName, profile: unlockModalPost.creatorInfo?.profile || unlockModalPost.user?.profile, }} post={{ publicId: unlockModalPost.publicId, text: unlockModalPost.text, price: unlockModalPost.price, }} onConfirm={handleConfirmUnlock} loading={unlocking} />
      )}
      {showPPVModal && activeStoreOwner && (
        <PPVRequestModal show={showPPVModal} onClose={() => setShowPPVModal(false)} creator={{ userId: activeStoreOwner?._id, displayName: activeStoreOwner?.displayName, userName: activeStoreOwner?.userName, profile: activeStoreOwner?.profile, }} post={{ _id: "" }} onSuccess={() => setShowPPVModal(false)} />
      )}
      {/* Video Modal */}
      <div className={`modal ${open ? "show" : ""}`} role="dialog" aria-modal="true">
        <div className="modal-wrap video_modal">
          ...
          <Plyr ref={playerRef} options={{ controls: ["play-large", "play", "progress", "current-time", "mute", "fullscreen"] }} source={{ type: "video", sources: [{ src: "https://res.cloudinary.com/drhj03nvv/video/upload/v1770026049/posts/69807440e60b526caa6da50c/1770026048286-screen-capture.webm.mkv", type: "video/webm", },], }} />
        </div>
      </div>
      {/* Crop Modal */}
...
//                       {loadingCreators ? (
//                         [1, 2, 3].map((i) => (
//                           <SwiperSlide key={`skeleton-${i}`} style={{ width: "auto" }}>
//                             <div className="creator-item loading-slide">
//                               <div className="icons_wrap"><div className="noprofile skeleton-pulse" /></div>
//                               <span className="skeleton-pulse" style={{ width: 48, height: 10, borderRadius: 4, display: "block" }} />
//                             </div>
//                           </SwiperSlide>
//                         ))
//                       ) : null}
